...
namespace NetBox.Admin.Persistence.Repositories.Security;

sealed class UserSecurityRespository : BaseRepository, IUserSecurityRespository
{
    ...
    public async Task<ResponseResult<UserDto>> CreateUser(string userName, string password, string email, string role, string firstName, string lastName, IEnumerable<string> permissions, string timeZone, string displayName, string? nicNumber, string? mobileNumber)
    {
        ...
        UserDto user = new(newUser.Id,
                           ...
                           new[] { role },
                           displayName,
                           newUser.UserProfile.NICNumber,
                           newUser.UserProfile.MobileNumber);
    ...
    public async Task<ApplicationUser?> FindByEmailOrUsername(string userEmailOrUserName, CancellationToken cancellationToken = new())
    {
        userEmailOrUserName = userEmailOrUserName.ToUpper();

        var user = await _dbContext.Users.FirstOrDefaultAsync(f => f.NormalizedEmail == userEmailOrUserName ||
                                                                   f.NormalizedUserName == userEmailOrUserName,
                                                                   cancellationToken);
    ...
    public async Task<ResponseResult> DeleteRole(Guid roleId, CancellationToken token)
    {
        ...
        bool isRoleInUse = await _dbContext.UserRoles.AnyAsync(a => a.RoleId == roleId);

        if (isRoleInUse) return new ResponseResult(new OperationFailedException("Role", "This role is in use"));
    ...
    public async Task<ResponseResult<UserRoleDto>> UpdateRole(Guid id, string roleName, CancellationToken token)
    {
        var role = await GetRoleById(id, token, asTracking: true);
        if (role is null) return new(new NotFoundException(nameof(id), "Role", id));

        if (role.IsDefault is false)
        {
            role.Name = roleName;
        }
    ...
    public async Task<IReadOnlyList<UserClaimsDto>> GetUserClaims(Guid userId, CancellationToken token)
    {
        ...
        var appClaims = AppModulePermissions.GetPermissionKeys();
