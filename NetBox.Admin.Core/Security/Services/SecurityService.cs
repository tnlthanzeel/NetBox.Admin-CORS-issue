...
using NetBox.Admin.Core.Security.Specs;
...
sealed class SecurityService : ISecurityService
{
    ...
    public async Task<ResponseResult<UserDto>> CreateUser(UserCreateDto model)
    {
        var validationResult = await _validator.ValidateAsync<UserCreateDtoValidator, UserCreateDto>(model, CancellationToken.None);
        ...
        await _userSecurityRepository.SaveChangesAsync();
    ...
    public async Task<ResponseResult<UserDto>> GetUser(Guid id, CancellationToken token)
    {
        ...
        UserDto user = new(appUser.Id,
                            ...
                            claims,
                            roles,
                            appUser.UserProfile.DisplayName,
                            appUser.UserProfile.NICNumber,
                            appUser.UserProfile.MobileNumber);
    ...
    public async Task<ResponseResult> UpdateUser(Guid id, UpdateUserDto model, CancellationToken token)
    {
        ...
        user.UpdateProfileInfoByAdmin(model.FirstName, model.LastName,model.DisplayName, model.NICNumber, model.MobileNumber);
    ...
    public async Task<ResponseResult> UpdateUserProfile(Guid userId, UpdateUserProfileDto model, CancellationToken token)
    {
        ...
        user.UpdateProfileInfoUser(model.FirstName, model.LastName);
